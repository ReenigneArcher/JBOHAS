...
if __name__ == '__main__':
	#argparse
	parser = argparse.ArgumentParser(description="Additional sensors and switches for Gogogate2 hub. Do you not use any quotes when entering arguments.", formatter_class=argparse.RawTextHelpFormatter)
	parser.add_argument('-i', '--ip', type=str, nargs=1, required=True, help='URL of wesbite... do not include http:// or /index.php')
	parser.add_argument('-u', '--user', type=str, nargs=1, required=False, default=['admin'], help='Username for logging into hub wesbite')
	parser.add_argument('-p', '--password', type=str, nargs=1, required=True, help='Password for logging into hub wesbite')
	parser.add_argument('-c', '--command', type=str, nargs=1, required=True, help='Command to execute. Options are light_status, light_on, light_off, door_status, door_operate')
	parser.add_argument('-d', '--door', type=int, nargs=1, required=False, default=[1], help='Door number... 1, 2, or 3')
	parser.add_argument('-m', '--move', type=int, nargs=1, required=False, default=[0], help='Beta function... integer required 0 = toggle, 1 = open, 2 = close... all options seem to toggle the door')
	parser.add_argument('-s', '--sensor', type=str, nargs=1, required=False, default=['temperature'], help='temperature or battery')
	...
	payload = {'login': user, 'pass': password, 'send-login': 'Sign+In'} #https://stackoverflow.com/a/28628514/11214013
