...
if __name__ == '__main__':
	#argparse
	parser = argparse.ArgumentParser(description="Additional sensors for Sindoh 3d printers.", formatter_class=argparse.RawTextHelpFormatter)
	parser.add_argument('-i', '--ip', type=str, nargs=1, required=True, help='IP address of printer.')
	parser.add_argument('-c', '--command', type=str, nargs=1, required=True, help='Command to execute. Options are printer_status, printer_cancel, printer_camera, printer_sensor')
	parser.add_argument('-s', '--sensor', type=str, nargs=1, required=False, default=['filename'], help='percent_complete, estimated_time_original, estiamted_time_remaining, filament_remaining, filament_rgb, filament_type, temperature_bed, temperature_nozzle, filename, printer_status_code')
	...
	if command == 'printer_status' or command == 'printer_sensor':
		...
		#print(sensor)
		#print(sensor_dictionary[sensor])
		#print(tmp[sensor_dictionary[sensor]])
